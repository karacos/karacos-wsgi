...
class ParentMeta(karacos.db['KcDocMeta']):
    ...
    def __call__(self, *args, **kw):
        ...
        """
        self.log.debug("BEGIN ParentMeta.__call__")
        instance = karacos.db['KcDocMeta'].__call__(self, *args, **kw)
        instance.log.debug("ParentMeta.__call__ : %s" % dir(instance))
        ...
        if 'childrens' not in instance:
            ...
            try:
                ...
                instance.parent.db[instance['_id']] = instance
            except:
                self.log.log_exc(sys.exc_info(),'warn')
        self.log.debug("END ParentMeta.__call__")
        return instance
...
class Childrens(dict):
    ...
    def __delitem__(self, name):
        ...
        assert name in self.parent['childrens'],"%s" % name
        del self.parent['childrens']['name']
        self.parent.save()
    
    def __setitem__(self, name, value):
        assert isinstance(value,basestring)
        assert isinstance(name,basestring)
        assert value in self.parent.db
        self.parent['childrens'] = value
        self.parent.save()
    
    def __getitem__(self, name):
        assert self.parent.child_exist(name), "KeyError, does't exist"
        result = self.parent.get_child_by_name(name)
        return result
    
    def __contains__(self,name):
        if name in self.parent['childrens']:
            return True
        else:
            return False
    def __iter__(self):
        """
        """
        return self.parent['childrens'].__iter__()
    def keys(self):
        return self.parent['childrens'].keys()
    def values(self):
        return self.parent['childrens'].values()
    def items(self):
        return self.parent['childrens'].items()
