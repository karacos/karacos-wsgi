...
import karacos
from karacos.lib import http, validate_since, file_generator_limited
...
def serve_file(path, content_type=None, disposition=None, name=None):
    ...
    # TODO: lines below gives problem...
    #validate_since()
    response.headers['Last-Modified'] = "%s" % http.HTTPDate(st.st_mtime)
    ...
    response.headers['Content-Type'] = "%s" % content_type[0]
    
    if disposition is not None:
        if name is None:
            name = os.path.basename(path)
        cd = '%s; filename="%s"' % (disposition, name)
        response.headers["Content-Disposition"] = "%s" % cd
    ...
    if request.environ['SERVER_PROTOCOL'] == 'HTTP/1.1':
        ...
        if r == []:
            ...
            raise karacos.http.HTTPError(416, message)
        if r:
            if len(r) == 1:
                ...
                for start, stop in r:
                    bodyfile.seek(start)
                    response.body = bodyfile.read(stop-start)
        else:
...
def staticdir(section, dir, root="", match="", content_types=None, index=""):
    ...
    if not os.path.normpath(filename).startswith(os.path.normpath(dir)):
        raise karacos.http.HTTPError(403) # Forbidden
